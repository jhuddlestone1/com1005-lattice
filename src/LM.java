...
public class LM{
  ...
  public int getCost(String w1, String w2) {
   ...
   if (w1.equals("*start*")) {return 0;};

   //find indices in vocab of w1 and w2

   while (i<vsize&&found==false){
       	if (vocab[i].equals(w1)){found=true;} else {i=i+1;};
       	};

   found = false;
   while (j<vsize&&found==false){
       	if (vocab[j].equals(w2)){found=true;} else {j=j+1;};
       	};
